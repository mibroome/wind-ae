...
- ``sim.windsoln.soln[]`` values are all as a function of radius (normalized values available via ``sim.windsoln.soln[]``):
   ...
   - Heating and Cooling Rates (per unit volume):
      - Included in the energy equation when their corresponding flag is on (see :ref:`relax_wind`'s :ref:`Heating & Cooling Flags <Heating & Cooling Flags>` and the ``Flags`` variables below):
