...
void get_spQ(double *spQ, I_EQNVARS vars,double k) {
  double n0overrho, nIONoverrho=0,nIONjoverrho=0, ne=0;
...
static double ion_eqn(int k, double *x, double **y, double ymod[NE+1][2], int species) {
  ...
  if (k==2){
     /* Approximate pressure scaleheight in units of Rp at base of simulation */
//         Hsc0 = K*kvars.T*T0*parameters.Rp / (parameters.molec_adjust*MH*G*parameters.Mp);
//        rate = 10/Hsc0;      
