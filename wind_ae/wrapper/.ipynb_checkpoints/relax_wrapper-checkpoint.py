...
#         self.width_factor = 1
    ...
    def load_planet(self, csv_file, calc_postfacto=True, name='Loaded Planet',print_atmo=True):
        '''
...
#         self.failed_deeper_bcs_ramp = False
        planet = wind_solution(file=csv_file, calc_postfacto=calc_postfacto)
...
#             print(output, error_output)
        ...
        sub = Popen(["cp", 'inputs/guess.inp', 'saves/windsoln.csv'],cwd=self.path, 
                    stdout=PIPE, stderr=PIPE)
        sub.wait()
        output, error_output = sub.communicate()
        if error_output:
...
#             self.load_spectrum()
    ...
    def save_planet(self, name=None, folder='', overwrite=False,polish=False,
                    repo=path+'saves/'):
        '''Note: It is not necessary to define a folder. 
        All files will be saved in the wind_ae/saves/ repo.
        '''
        if polish == True:
            self.polish_bcs()
        if folder != '':
    ...
    def run_wind(self, expedite=False, calc_postfacto=True, verbose=False, retry=False):
        ...
        if error_output:
            ...
            self.windsoln = wind_solution(calc_postfacto=calc_postfacto)
...
#Ramping Functions
    def ramp_to(self, system=None, converge_final_bcs=False,intermediate_converge=False,
                integrate_out=False,static_bcs=False,make_plot=False): #physics=None, bcs=None,
        '''Description: Ramps all parameters (ramp_flux, ramp_grav, ramp_star, etc.). If ramping
                        directly using ramp_to() does not work, try running ramp_flux, ramp_grav,
                        and ramp_star in a different order.

        Arguments: system - obj; system object, accepts system = system(Mp,Rp,Mstar,a,Ftot,Lstar)
                   converge_final_bcs - bool; True runs polish_bcs(converge_Rmax=integrate_out) 
                                               after the final ramping step.
                   intermediate_converge - bool; True runs polish_bcs(converge_Rmax=integrate_out)
                                                 after each ramping step
                   integrate_out - bool; True requires that final solution is integrated past sonic point 
                                         to Coriolis radius.
                   static_bcs - bool; True maintains the existing boundary conditions in the solution
                                      (Lower: Rmin, rho, T; Upper: Ncol_sp)
                   make_plot - bool; True plots snapshots of T, v, rho, ion frac vs. r for some 
                                     intermediate solutions in the ramping process.
        Returns: 
                0 - Success
                1 - Relaxation code failed to converge at some point in the ramping process.
                5 - Ramping was successful, but polishing boundary conditions to self consistency failed.
        '''
        ...
        if system is not None:
            ...
            result = self.ramp_var("Ftot", system.value("Ftot"),
                                  converge_bcs=False, make_plot=make_plot,
                                  expedite=True,
                                  integrate_out=False)
            fail = result           
            if (fail != 0) and (fail != 5):
                return fail
            elif intermediate_converge == True:
                self.polish_bcs(converge_Rmax=False,static_bcs=static_bcs,print_final=False)            
            
            # Ramps Mp and Rp simultaneously, ~constant surface gravity
            result = self.ramp_grav(system, converge_bcs=False,
            ...
            result = self.ramp_star(system, converge_bcs=False,
                                   make_plot=make_plot, expedite=True,
                                   integrate_out=False)
            fail = result
            if intermediate_converge == True:
                self.polish_bcs(converge_Rmax=integrate_out,static_bcs=static_bcs,print_final=True)            
            elif (intermediate_converge == False) & (converge_bcs==True):
                self.polish_bcs(converge_Rmax=integrate_out,static_bcs=static_bcs,print_final=True)                
            elif (integrate_out == True) & (intermediate_converge == False) & (converge_bcs == False):
                self.converge_Rmax()
...
#         if all(np.array([prct_Mp,prct_Rp])<0.2) and (self.windsoln.nspecies<4):
#             #too costly to try to jump directly and fail if nspecies is high
#             print("\rAttempting to ramp directly.",end="               ")
#             self.inputs.write_planet_params(end_Mp,end_Rp,*self.windsoln.planet_tuple[2:])
#             if self.run_wind(expedite=True,calc_postfacto=False)==0:
#                 srt_Mp = end_Mp
#                 srt_Rp = end_Rp
#                 print(f"\rSuccessfully ramped directly to {end_Mp:.3e}g & {end_Rp:.3e}cm.",end="               ")
        if srt_Mp != end_Mp:
    ...
    def add_metals(self, desired_species_list, Z=1, custom_mfs=[], Ncol_sp_tot=0, 
        ...
        if result==0:
            ...
            if result == 0:
                ...
                print("Failed to ramp mass fracs directly to goal. ")
...
#             self.windsoln.nspecies -= len(remove_species_list)
            self.load_planet(self.path+'inputs/guess.inp',calc_postfacto=False,print_atmo=False)
        except ValueError:
...
#precision, but are for maximal accuracy (within the inherent uncertainty in the model))
    def polish_bcs(self,converge_Rmax=True,static_bcs=False,print_final=True):
        ...
        #--------------
        isotherm = self.run_isotherm(polish=True,quiet=False) #temporary fix
        ...
        if print_final==True:
            ...
            quiet=True
        self.windsoln.add_user_vars(expedite=True,quiet=quiet) #do postfacto calcs to get 'heat_ion' & 'boloheat'
        while len(np.where(self.windsoln.soln['heat_ion'][idx:]<self.windsoln.soln['boloheat'][idx:])[0]) > 20:
            ...
            self.windsoln.add_user_vars(expedite=True,quiet=True)
...
#                 print('\rConverging Rmax failed, retrying by converging Ncol_sp first.',end='                  ')
                Ncol = 0
#                 Rmax = self.converge_Rmax()
            if Rmax==0:
                Ncol=0 
        ...
        warn1=f'-\nIntermed. polish: ramp_base_bcs=%s, ramp_isotherm=%s, converge_Ncol_sp=%s'
        fails = 0
        if avg_diff > 1e-2:
            fails += 1
        ...
        if fails == 0:
            ...
            if converge_Rmax == True:
                ...
                if print_final==True:
                    ...
                    print(warn1 %(bc_result,iso_result,ncol_result))
            return 5
...
#             self.windsoln.bcs_tuple[5] = goal
            ...
            while result != 0:
                if result == 4:
                    print("Rmax done, but Ncol cannot be lowered to self-consistent value without introducing integration errors. Reverting Ncol. Maximum error in Mdot in this case is 10 percent.")
                    #returning to last outward integrated solution
                    self.inputs.write_bcs(*self.windsoln.bcs_tuple[:-2],
                                         goal*1.5,
                                         self.windsoln.bcs_tuple[-1])
        ...
        self.inputs.write_flags(*self.windsoln.flags_tuple,integrate_out=True)
        self.windsoln.add_user_vars(quiet=True)
        return 0
    

    def converge_Ncol_sp(self,expedite=False,quiet=False,polish=False):
        """
        Description:
        ...
        Inputs:
            ...
            quiet - bool; default = False. Prints warnings about Ncol_sp being only estimates if
                                            integrate_out = False.
        """
        integrate_out = self.windsoln.flags_tuple[-1] 
...
#         if polish==True:
        ...
        if niter != 0:
            ...
            if self.run_wind(expedite=expedite,calc_postfacto=False) == 4:
                if quiet == False:
                ...
                return 4
            elif self.run_wind(expedite=expedite,calc_postfacto=False) == 1:
        ...
        if expedite==False:
            ...
            if self.run_wind()!=0:
                if quiet == False:
                    print("Failed to integrate past sonic point.")
                return 1
            else:
                return 0
...
#             n_neutral *= self.windsoln.soln['Ys_'+self.windsoln.species_list[j]]
    ...
    def run_isotherm(self,width_factor=0.,return_idx=False,called_in_ramp_bcs=False,
        ...
        self.load_planet(self.path+'saves/windsoln.csv',calc_postfacto=False,print_atmo=False)
...
#         if drop_index > 800: #low grav superearths need no drop off
#             rate = rate_calc(drop_index,3)
#         else:
