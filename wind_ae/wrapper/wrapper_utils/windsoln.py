...
class wind_solution:
    ...
    savepath = pkg_resources.files("wind_ae").joinpath("saves/windsoln.csv")

    def __init__(self, file=savepath, calc_postfacto=False, print_warnings=True):
    ...
    def add_user_vars(self, expedite=False):
        ...
        if expedite is False:
            ...
            self.R_hill = self.semimajor_normed * (self.Mp / (3.0 * self.Mstar)) ** (
                1.0 / 3.0
            )
        ...
        Ncol_arr[Ncol_arr < 0] = (
            0  # less than 0 because unconverged soln returns negative Ncols. This feels sus.
        )
        ...
        wPhi = (
            np.multiply(
                np.tile(self.sim_spectrum["wPhi"], (len(taus), 1)), np.exp(-taus)
            )
            * self.Ftot
        )
        ...
        frac_in_heat = 0.9971 * (
            1 - pow(1 - pow(background_ioniz_frac, 0.2663), 1.3163)
        )
        ...
        for s, species in enumerate(self.species):  # 48s
            ...
            f = np.nan_to_num(
                sig_matrix * Ncol_matrix / taus
            )  # frac of incoming photon energy that will interact with species s
            E0_matrix = E_matrix - self.ion_pot[s]

            heatfrac_matrix = (np.tile(frac_in_heat, (len(self.E_wl), 1))).T
            heatfrac_matrix[E0_matrix < 6.408707e-11] = (
                1  # zeroing where E_0 too low for secondary ionizations (40eV)
            )
            # technically should go to lower eV, but Shull & Steenberg analytic eq. is only valid down to 100 eV, we test and find that 40 eV makes little difference to results.
            for m in range(self.nspecies):
                # sigma_m = sigmas.iloc[:, m]
                sig_matrix_m = np.tile(
                    sigmas.iloc[:, m], (len(background_ioniz_frac), 1)
                )
                sig_matrix_m_Hydrogen = np.tile(
                    sigmas.iloc[:, 0], (len(background_ioniz_frac), 1)
                )
        ...
        if expedite is False:
            ...
            for s, sp in enumerate(self.species_list):
                ...
                self.I_mean += self.ion_pot[s] * integrate.simps(
            ...
            self.eff2 = integrate.simps(self.soln["heat_ion"], self.soln["r"]) / self.F0
            ...
            if (self.species_list[0]).replace(" ", "") != "HI":
                if self.print_warnings:
                    print(
                        "WARNING: Calculation of Lyman alpha cooling assumes Hydrogen is the first species listed."
                    )
            ...
            )  # is this too many elextrons when we have other species? Should there be in some limit <= n_H?
            filepath = pkg_resources.files("wind_ae.wrapper.wrapper_utils").joinpath(
        ...
        nom = 1.0 + self.ion_wgt(1.0 - self.soln["Ys_HI"]) * (1.5 + I_kT)  # FIX?
        dom = 2.5 + self.ion_wgt(1.0 - self.soln["Ys_HI"]) * (3.75 + 5 * I_kT + I_kT**2)
        ...
        nom = 2.0 + 2.0 * self.ion_wgt(1.0 - self.soln["Ys_HI"]) * (1.5 + I_kT)
        dom = 3.0 + 2.0 * self.ion_wgt(1.0 - self.soln["Ys_HI"]) * (1.5 + I_kT) ** 2
    ...
    def calc_efficiency(self):
        ...
        for i, r_idx in enumerate(rs):
            heat_eff = np.zeros_like(self.E_wl)
            ion_eff = np.zeros_like(self.E_wl)
            for wl in range(len(self.E_wl)):
                sigma = self.sigma_wl[wl]
                taus = self.taus[r_idx, wl]
                ...
                for j, species in enumerate(self.species_list):
                    ...
                    Ncol = self.soln["Ncol_" + species][
                        r_idx
                    ]  # some point in wind - can experiment
                    f_j = Ncol * sigma[j] / taus
                    # F_in_species_j = f_j * F
                    E0 += (E - self.ion_pot[j]) * f_j

                # technically should go to lower eV, but Shull & Steenberg analytic eq. is only valid down to 100 eV, we test and find that 40 eV makes little difference to results.
                if E > 6.408707e-11:  # 40 eV
                    f_heat = self.frac_in_heat[r_idx]
                else:
                    f_heat = 1

                heat_eff[wl] = E0 * f_heat / E
                ion_eff[wl] = 1 - E0 * f_heat / E

            avg = np.append(avg, np.mean(heat_eff))
        return np.mean(avg)
    ...
    def calc_fits(self):
        ...
        stime = (
            integrate.cumtrapz(1.0 / self.v_fit(r_int), r_int, initial=0)
            * self.scales[0]
        )
    ...
    def calc_Coriolis(self):
        ...
        if self.cori.status:
            ...
            cori_x = -s_int[0] + integrate.cumtrapz(
                self.cori.sol(s_int)[0] / self.v_fit(s_int), x=s_int, initial=0
            )
            cori_y = integrate.cumtrapz(
