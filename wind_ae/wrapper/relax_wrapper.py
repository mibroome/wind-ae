...
#         self.failed_deeper_bcs_ramp = False
    ...
    def load_spectrum(self, generate=False, wl_norm=1e-7,print_warnings=False):
        '''Description: Loads a high resolution spectrum and, if generate=True,
        smooths it, keeping the flux at ionization edges for the species in the current 
        windsoln accurate, and conserving flux locally in the vicinity of spectral peaks. 
        The spectrum specified in the windsoln is the one loaded and smooth. 
        
        One can change the spectrum using ramp_to_user_spectrum() to
        ramp to a user-input spectrum or change the wavelength range and integrated flux via 
        ramp_spectrum().To change the total flux, but maintain spectrum shape and range, use
        ramp_flux().
        ...
        Arguments: 
            ...
            print_warnings - bool; if True, prints the warnings from 
                                    McAstro/planets/insolation/glq_spectrum.csv.
            '''
    ...
    def run_wind(self, expedite=False, calc_postfacto=True, verbose=False, retry=False):
        '''Description: Runs C relaxation code. If successful, returns 0, saves solution in 
                        saves/windsoln.csv and copies this solution to be the next guess in
                        inputs/guess.inp. 
        
           Arguments: 
               expedite - bool; default=False. If true will also run outward integration.
                               Will return 4 if relaxation code has solved, but outward 
                               integration fails (does not affect profile or mass loss accuracy).
               calc_postfacto - bool; default=True. If true takes a couple of seconds to 
                                       perform post-facto calculations (e.g., heating/cooling,
                                       number density, etc.)
               verbose - bool; default=False. 
               retry - bool; default=False. 
               
           Returns:
               0 - if expedite=False, both relaxation and outward integration have succeeded.
                   if expedite=True, relaxation has succeeded. saves/windsoln.csv has been copied
                   to inputs/guess.inp and new solution has been loaded in the windsoln object.
               4 - relaxation has succeeded, but outward integration has failed.
               1 - relaxation has failed
               2 - there has been an error in copying saves/windsoln.csv to inputs/guess.inp (rare)
               3 - there has been an analysis error when excecuting wind_solution() to load current
                   solution into windsoln object
        '''
...
#                     self.ramp_base_bcs(expedite=expedite,static=static_bcs) #if expedite = False, converges bcs, too #commented out 8-28-24 b/c have never seen retry triggered and it wouldn't work with static bcs
...
#Ramping Functions
    def ramp_to(self, system=None, converge_bcs=False,intermediate_polish=False,
        ...
        if system is not None:
            ...
            result = self.ramp_var("Ftot", system.value("Ftot"),
                                  converge_bcs=converge_bcs, make_plot=make_plot,
                                  expedite=True,
                                  integrate_out=False)
            fail = result
            ...
            result = self.ramp_grav(system, converge_bcs=converge_bcs,
            ...
            result = self.ramp_star(system, converge_bcs=converge_bcs,
                                   make_plot=make_plot, expedite=True,
                                   integrate_out=False)
            fail = result
...
#                     self.converge_Ncol_sp() #MAYBE
    ...
    def ramp_grav(self, system, delta=0.02, converge_bcs=False, make_plot=True,
                  expedite=False,integrate_out=True,static_bcs=False):
        """Description: Ramps planet mass and radius along lines of constant surface gravity
                        with adaptive stepsizes.
        
        Arguments:
            system - obj; system objects take Mp, Rp, Mstar, semimajor, Ftot, Lstar in cgs units.
            delta - float; default=0.02. Default stepsize. Unlikely to need to change.
            converge_bcs - bool; if true, will run polish_bcs(), which can be costly but may
                                 improve liklihood of convergence.
            make_plot - bool; if True, will plot temperature, velocity, density, and ionization 
                              fraction profiles at steps throughout the ramp. (Jupyter notebooks
                              do not allow for live plotting, so will appear at end).
            expedite - bool; if True, will skip integrating out to improve speed. 
            integrate_out - bool; if True, will require integrate out to R_cori at the end
                                  of the ramp. NOTE: R_cori will still need to be computed by
                                  running converge_Rmax().
            static_bcs - bool; if True, skips running ramp_base_bcs() when ramper is having
                               difficulty converging. Useful when not wanting to use default BCs.
         
         Returns:
             0 - Successfully ramped (and integrated out and/or converged BCs)
             101 - Failed to ramp to desired Mp, Rp
             1 - Unusual. Succesfully ramped to Mp, Rp then encountered trouble solving. 
                 Last working solution should be sufficient.
             4 - Succesfully ramped to Mp, Rp, but unable to integrate out. Try running 
                 converge_Rmax() or integrate_out().
             5 - Succesfully ramped to Mp, Rp, but unable to polish all BCs. See printout 
                 for more details.                  
        """
#         self.failed_deeper_bcs_ramp = False
...
#                     self.converge_Ncol_sp()
    ...
    def ramp_star(self, system, delta=0.02, converge_bcs=True, make_plot=True,
        ...
        '''Description: Ramps stellar mass (Mstar), semimajor axis (semimajor), and stellar bolometric 
                        ...
                        If semimajor axis and Lstar change, ramps linearly along optical 
                        flux (essentially along skin temperature since T_skin propto Fopt) 
                        rate of change.
        ...
        Arguments:
         ...
         Returns:
             ...
             1 - Unusual. Succesfully ramped to Mstar, Lstar, semimajor then encountered 
                 trouble solving. 
                 Last working solution should be sufficient (self.path+'saves/windsoln.csv').
             4 - Succesfully ramped to Mstar, Lstar, semimajor but unable to integrate out. 
                 Try running converge_Rmax() or integrate_out().
             5 - Succesfully ramped to Mstar, Lstar, semimajor but unable to polish all BCs. 
...
#Metals functions    
    def metallicity(self,metals_list,Z):
        """Description: Returns an array of the Lodders (2009) Z X solar metallicity MASS 
        fractions for the metals in metals_list. Mass frac must sum to 1, so difference from 1 is
        added to the hydrogen mass fraction (always at index 0 in the array). 
    ...
    def add_metals(self, desired_species_list, Z=1, custom_mfs=[], Ncol_sp_tot=0, 
        ...
        '''Description:
                ...
                of species already in the solution use ramp_metallicity(). Skips adding
                if species is already present. This function will converge to a self-
                consistent Ncol_sp for each species at the final step if converge_Ncol_sp=True.
           Inputs:
               ...
               integrate_out  - bool; takes time to integrate past sonic point.       
           Returns:
               None
        '''
        self.metals = metal_class(self.windsoln)
        ...
        if self.run_wind(expedite=True)==0:
            print("\rMetals added to guess, now ramping mass fractions.",end="                      ")
            ...
            if result == 0:
                if Z>1:
                    print(f"\rMetals successfully added in at Z=1. Ramping to Z={Z}.")
                    ...
                    if converge_Ncol_sp==True:
                        print("Successfully ramped mass fracs. Integrating out & converging Ncol_sp...")
                    ...
                    if converge_Ncol_sp==False:
                        self.inputs.write_flags(*self.windsoln.flags_tuple,
                                                integrate_out=True)
                        print("Successfully ramped mass fractions. Integrating out, but not converging Ncol_sp.")
                    ...
                    if converge_Ncol_sp==True:
                        print("Successfully ramped mass fracs. Converging Ncol_sp, but not integrating out...")
                        return self.converge_Ncol_sp()
                    else:
                        print("\rSuccessfully ramped mass fractions.",end="                      ")
                        return 0
            else:
                print("Failed to ramp mass fracs directly to goal. Taking smaller steps. ")
                #put stepper function in here
                #
                #
                #
                return result
        else:
            print("Failed to add new species. That shouldn't have happened. Try polishing solution before adding metals.")
            return result

        
    
    def remove_metals(self, remove_species_list, run_wind=True):
        """Description: Removes a metal from the list. Note: He will take a signficant amount 
        of time to remove, so it is advisible to ramp directly to your desired solution 
        from an existing pure-H solution.
        """
        self.metals = metal_class(self.windsoln)
        try:
...
#             self.windsoln.nspecies -= len(remove_species_list)
    ...
    def ramp_metallicity(self,Z=1,custom_mfs=[],slow_ramp=False,static_bcs=False,
                        integrate_out=True, converge_Ncol_sp=True):
        '''Description: Ramps up the metallicity of the species present in the simulation.
           ...
           Arguments: 
               Z - float; default=1. Multiples of Lodders (2008) solar metallicity.
               custom_mfs - list of floats; default=[]. If [], will default to Z. 
                                           If not empty, will use the custom_mfs.
        '''
        goal_Z = Z
        if goal_Z >= 50:
            print("Note: For high metallicities, consider increasing mean molecular weight (molec_adjust).")
            for sp in self.windsoln.species_list:
                if (sp[:2] == 'Fe') or (sp[:2] == 'Ca'):
                    print("WARNING: At high Z, Fe and Ca line cooling are significant, but have not yet been implemented.")
            print("\n")
        ...
        if len(custom_mfs) != 0:
            ...
            goal_nfs = ratio/sum(ratio)
            if goal_nfs[0] < 0.5:
                print("NOTE: Secondary ionization calcs assume H is the dominant species, therefore may not be accurate.")
            self.inputs.write_bcs(*self.windsoln.bcs_tuple[:-2],
                                 0.5*goal_nfs, #an arbitrary value that works well for ramping
                                 self.windsoln.bcs_tuple[-1])
            if self.run_wind(expedite=True,calc_postfacto=False) == 0:
            ...
            while (1-current_Z/goal_Z) > 1e-5:
                if abs(goal_Z - current_Z) > 10:
                    ...
                    step_Z = goal_Z
                print(f'\r Success! Attemping to ramp Z from {current_Z:.1f} to {step_Z:.1f}',
                 end='                                                                               ')
...
#in the model))
    def polish_bcs(self,converge_Rmax=True,static_bcs=False):
        '''Description: Polishes upper and lower boundary conditions to self-consistency.
            ...
            Arguments: 
                converge_Rmax - bool; default=True. If true converges Rmax (more costly).
                static_bcs - bool; if True, skips running ramp_base_bcs() and maintains the
                             current boundary conditions.
                             
            Returns:
                0 - Successfully polished all BCs.
                5 - Failed at polishing one or more BCs. See printout for more details.
                    If "Molec. to atomic transition" has failed, plot energy_plot() to
                    assess whether the current solution is satisfactory.
        '''
        ...
        while len(np.where(self.windsoln.soln['heat_ion'][idx:]<self.windsoln.soln['boloheat'][idx:])[0]) > 20:
            width /= 2
            print(f"\r...Shortening transition from molecular to atomic region. Erf width = {width:.2f} Hsc",
                 end="                                                                ")
            if self.run_isotherm(polish=True,width_factor=width) != 0:
                print("Attempting to shorten erfc transition region between molecular and atomic regions",
        ...
        while any(-self.windsoln.soln['heat_advect'][idx-10:idx+10]>
            ...
            if width > 10:
                ...
                isotherm=1
                break
            print(f"\r...Smoothing transition from molecular to atomic region. Erf width = {width:.2f} Hsc",
                 end="                                                                ")
...
#                 Rmax = self.converge_Rmax()
    ...
    def ramp_base_bcs(self,base_press=1,
        ...
        Arguments:
            ...
            polish - bool; default=False. If true, skips ramping T when 
                            self.windsoln.bolo_heat_cool=0 to avoid costly iteration.
        '''   
        if static==True:
            return
...
#         #this is because some high flux planets may need lower lower BC to capture all heating
    ...
    def converge_Rmax(self,final_converge_Ncol=True):
        ...
        if final_converge_Ncol == True:
            ...
            if result == 3:
                print(f"Successfully converged Rmax to {self.windsoln.Rmax:.6f} (To preserve outward integration, Ncol was slightly increased => Mdot error < ~10%).")
        else:
            print(f"\rSuccessfully converged Rmax to {self.windsoln.Rmax:.6f}. It is recommended to run converge_Ncol_sp(integrate_out=True) as Ncol will have updated.",
                  end="                                                       ")
    ...
    def converge_Ncol_sp(self,expedite=False,integrate_out=False,quiet=False):
            ...
            while result != 0:
                ...
                if result == 4:
                    if integrate_out == False:
                        ...
                        print("\rIntegration error. To preserve outward integration, raising Ncol_sp slightly and returning.",
                             end="                                                          ")
                        self.inputs.write_flags(*self.windsoln.flags_tuple,integrate_out=True)
                        self.inputs.write_bcs(*self.windsoln.bcs_tuple[:-2],
                                             Ncol_goal*1.1,
                        ...
                        while result==4:
                            fail+=1
                            print(f"\rAttempting to integrate out. Ncol_sp x{(1+(fail+1)/10):.1f}",
                                  end="                                         ")
                            self.inputs.write_bcs(*self.windsoln.bcs_tuple[:-2],
                        ...
                        if result == 0:
                            if fail == 0:
                                return 0  
                            if fail > 0:
                                print(f"\rSuccessfully integrated out w/ Ncol_sp = {(1+(fail)/10):.1f} x goal Ncol_sp. Error in Mdot < ~10%.",
                                      end="                                 ")
                                return 3
                else:
        ...
        #Final convergence of all species at once (MAY NOT WANT THIS BECAUSE WILL MOVE IT again)
        if niter != 0:
            if expedite == True:
                if quiet == False:
    ...
    def run_isotherm(self,width_factor=0.,return_idx=False,called_in_ramp_bcs=False,
                     polish=False):
                ...
                if (self.skip == False) or (polish == True):
                    while np.mean(abs(goal_erfs - current_erfs)/goal_erfs) > 1e-5:
                        ...
                        while self.run_wind(expedite=True,calc_postfacto=False) != 0:
                            ...
                            elif (fail<10) : #try taking a smaller step
            ...
            if len(np.where(-cool[:20]<heat[:20])[0]) > 3:
                ...
                while self.windsoln.bolo_heat_cool < 1:
                    ...
                    while self.run_wind(calc_postfacto=False) != 0:
                        ...
                        print(f'\rRamping back in bolometric heating & cooling. Trying factor of {flags[2]:.2f}',
                      end='                                                  ')
                        self.inputs.write_flags(*flags)
                        if fail>10:
...
#                 return 1
    ...
    def turn_off_bolo(self):
        '''Description: Turns off bolometric heating and cooling AND the mean molecular weight
        adjustment in the molecular region. (These values are currently coupled by the error 
        function defined in run_isotherm()).
        '''
        self.load_planet(self.path+'saves/windsoln.csv',calc_postfacto=False,
                         print_atmo=False,print_warnings=False)
    ...
    def erf_velocity(self,return_idx=False,called_in_ramp_bcs=False, 
        ...
        '''Description: Defines the drop-off radius at which the complementary 
           ...
           Arguments: return_idx - bool; default=False. If true, returns radial
                      ...
                      polish - bool; default=False. If true, computes full bolometric 
                                      heating/cooling and photoionization heating curves.
                      width_factor - float; default=0. Sets width in radius space over which the 
                                           erfc drop off as a factor 
                                           of scaleheight at the transition index (Hsc). 
                                           If 0, width_factor = current width of transition in Hsc.
           Returns:
               v[drop_index] - velocity of drop-off in units of cm/s.
               rate_calc(drop_index,width_factor) - velocity of drop-off in units of cm/s.
               
               if return_idx=True:
                   width_factor - in units of Hsc
                   drop_idx - radial index at which (photoionization heating) > |PdV cooling|
...
#         print('drop index',drop_index)
    ...
    def raise_Ncol_sp(self,to_total=0.8,by_factor=0,expedite=False):
        """Description: Increase per_species column density at sonic point.
                        This is distinct from converging Ncol_sp. 
                        Too low of Ncol_sp may cause numerical instability, so if
                        ramping is stuck, try raising Ncol_sp. Can also diagnose
                        by plotting six_panel_plot--if Ncol_sp drops off sharply after
                        the sonic point for several species, this may be the source of
                        numerical instability.
        
        Arguments: 
            to_total - float; if by_factor=0, then raises sum(Ncol_sp) until =to_total
            by_factor - float; if != 0, then multiplies Ncol_sp x factor
            expedite - bool; if true, does not integrate out 
        """
        if by_factor == 0:
            self.inputs.write_bcs(*self.windsoln.bcs_tuple[:-2],
    ...
    def turn_off_tidal_grav(self):
        '''Description: Turns off the tidal gravity.
        '''
        flag = self.windsoln.flags_tuple[1]
        stepsize=0.1
        while np.round(flag,4) > 0:
            flag+= -stepsize    
            print(f'\rTrying {flag:.4f} x tidal grav term',end='                ')
            ...
            if self.run_wind() == 0:
                ...
                stepsize/=10 
                print(f'Failed. Trying smaller stepsize {stepsize:.4f}.')
        print('Success! Tidal gravity turned off.')
        return
    ...
    def ramp_rho_rmin(self,goal_rho,integrate_out=False):
        ...
        while (abs(1.-self.windsoln.rho_rmin/goal_rho) > 1e-10):
            ...
            if OOM_old != OOM_current:
                print(f'..Order of mag of rho_rmin has changed, C source code does not update dynamically. If run takes a long time or fails to converge, halt run and restart ramp_rho_rmin() to update rho_rmin convergence scale.',
                      end="                                                                      ")            
        ...
        while abs(sum((goal_span-self.windsoln.spec_resolved/self.spectrum.wl_norm)/goal_span)) > 1e-3:
            ...
            while self.run_wind(expedite=True,calc_postfacto=False) != 0:
                ...
                if fail > 6:
                    ...
                    print(f'Failure: Too many attempts. Final range: [{curr[0]:.2f},{curr[1]:.2f}]nm ([{eV[1]:.2f},{eV[0]:.2f}]eV).')
                    ...
                    if normalize == True:
                        ...
                        result =  self.flux_norm(Fnorm,ranges,
                                               ramp=True,plot=plot,integrate_out=True,converge_bcs=False)
                        curr = self.windsoln.spec_resolved/self.spectrum.wl_norm
                        print(f'Final Ftot at planet across [{curr[0]:.2f},{curr[1]:.2f}]nm = {self.windsoln.Ftot:.0f} ergs/s/cm2.')
                    return 1
                curr = self.windsoln.spec_resolved/self.spectrum.wl_norm
    ...
    def ramp_to_user_spectrum(self,spectrum_filename,species_list=[],
        ...
        Arguments:
            ...
            species_list - list of str; default = [] sets to species list of 
                                    the loaded windsoln.
            updated_F - float; Summed flux over the norm_spec_range in ergs/s/cm2 
                                AT SEMIMAJOR AXIS OF PLANET.
            ...
            kind - str; 'full' or 'mono' - spectrum frequency type
            plot - bool; default = False.
            ramp_range - bool; default = True. If false will not ramp spec range or flux.
            hires_savgol_window - ODD int; Length of window in Savitzky–Golay smoothing if 
                                  user input spectrum has >10,000 wavelength bins. 
        ...
        h[2] = '%d\n' %hires_savgol_window
        f.close()
        g = open('McAstro/stars/spectrum/additional_spectra/'+spectrum_filename,'w')
        g.writelines(h)
        g.close()
        ...
        print("*** REMINDER: Goal spectrum (orange) oversmoothed? Lower the hires_savgol_window value. ***")
...
#         if len(comment) > 0:
        ...
        if wl_cm[-1] > 1e-4:
            print(f"WARNING: Double check - Input wavelength array might not be in units of {wl_units:s}.")
    ...
    def four_panel_plot(self,Mdot_legend=True,c='k',ls='-',label='',label_dim=[0,1.3,2],
                        ion_label=True,first_plotted=True,ax=0):
        four_panel_plot(self.windsoln,Mdot_legend,c,ls,label,label_dim,
                        ion_label,first_plotted,ax)
        return
    
    def integrate_out(self):
        '''Description: Forces solution to integrate out past sonic point to the Rmax. 
                        May not preserve self-consistency of per-species column density at sonic 
                        point (Ncol_sp) as it may be necessary to raise Ncol_sp in order to 
                        integrate outwards.
        
                        By default Rmax is set to currently-computed R_cori, but can be manually 
                        set Rmax to desired value via:
        ...
        if result==4:
            ...
            print(f"\rRaising Ncol_sp x{factor:d} to allow outward integration to the currently computed R_cori.",end="                                                                       ")
