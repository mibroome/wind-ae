...
//                     }
  ...
  for (j=0; j<NSPECIES; j++){
      ion_rate = vars.Ys[j]*parameters.Fuv*glq_ionization(vars.Ncol,vars.Ys,vars.rho,k)[j]; 
...
/*============================================================================*
 *! \fn static void get_spQ(double *spQ, I_EQNVARS vars)                      *
 *  \brief Calculates the total specific heating                              *
 *----------------------------------------------------------------------------*/

void get_spQ(double *spQ, I_EQNVARS vars,double k) {
  double n0overrho, nIONoverrho=0, ne=0;
  double spQ_photoheat=0.0, spQ_lyacool=0.0, spQ_reccool=0.0;
  int j,sp;
  ...
  *spQ = 0.0;
  ...
  for (j=0; j<NSPECIES; j++){
      ...
      spQ_photoheat  += n0overrho*parameters.Fuv*glq_heating(vars.Ncol,vars.Ys,vars.rho,k)[j];
    } 
//   spQ_photoheat  = nHIoverrho*parameters.Fuv*glq_heating(Ncols)[0] + nZoverrho*parameters.Fuv*glq_heating(Ncols)[1];
  ...
  *spQ += spQ_photoheat;
  ...
  if (parameters.lyacool == ON) {
    ...
    spQ_lyacool  = LYACOOL_COEFF*exp(-LYACOOL_TEMP/vars.T)*(n0overrho)*ne;  
...
//   }
  ...
  if (OFF) {
    ...
    spQ_reccool  = RECCOOL_COEFF*pow(vars.T, 0.11)*(nIONoverrho)*ne;
