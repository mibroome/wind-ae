        ...
        with open(file, 'r') as f:
            ...
            for line in f:
                ...
                if line[0] == '#nspecies:':
                    ...
                    flags = [int(x) for x in line[1:]]
                    # flags
                    self.lyacool = flags[0]
                    self.tidalforce = flags[1]
                    if len(flags) > 2:
                        self.integrate_inward = flags[2]
                        self.integrate_outward = flags[3]
                    else:
                        self.integrate_inward = 0
                        self.integrate_outward = 1
...
#                         self.species[s] = f'{line[4*(s+1)].split("}")[0]}'
        ...
        self.planet_tuple = [self.Mp, self.Rp, self.Mstar, self.semimajor,
                             self.Fuv,self.Lstar]
        self.physics_tuple = [self.HX, self.species_list, self.atomic_masses] 
        ...
        self.flags_tuple = [self.lyacool, self.tidalforce,
                            self.integrate_inward, self.integrate_outward]
        ...
        # set critical values
        self.r_crit = 1.+self.soln_norm['z'][0]
        self.crit_index = len(self.soln[self.soln_norm['r'] < self.r_crit])-1
...
#         start = time.time()
        ...
        self.Phi0 = self.Fuv*self.sim_spectrum['wPhi'].sum()
        self.F0 = self.Fuv*(self.sim_spectrum['E']
...
#             print(col_name)
        ...
        self.soln['mu'] = const.mH/mu_denominator
                #         self.soln['mu'] = const.mH/(0.25 + self.HX*(1.75-self.soln['Ys'])) //single species H case
        # Gas pressure
        ...
        for j in range(self.nspecies):
            ...
            self.soln[neutral] = self.soln['Ys_'+self.species_list[j]]*self.soln[total]
            self.soln[ionized] = self.soln[total] - self.soln[neutral]
            self.soln['n_e'] += self.soln[ionized]
        ...
        background_ioniz_frac[background_ioniz_frac<0] = 0
        ...
        wPhi = np.multiply(np.tile(self.sim_spectrum['wPhi'],(len(taus),1)),np.exp(-taus))*self.Fuv
        ...
        for s, species in enumerate(self.species): #48s
            ...
            heatfrac_matrix[E0_matrix<1.6022e-10] = 1 #zeroing where E_0 too low for secondary ionizations (100eV)
            ...
            n_abs = self.soln['n_'+species]*self.soln['Ys_'+species] #FIX
        ...
        tau_ion_header = ''
        for s in range(self.nspecies):
            ion_header += f'ionization_{self.species[s]},'
            ion_eq_header += f'ion_eq_{self.species[s]},'
            tau_ion_header += f'tau_ion_{self.species[s]},'
        ion_header = ion_header.split(',')[:-1]
        ion_eq_header = ion_eq_header.split(',')[:-1]
        tau_ion_header = tau_ion_header.split(',')[:-1]
        heat_header = ''
        for s in range(self.nspecies):
            heat_header += f'heating_{self.species[s]},'
        ...
        self.I_mean = 0
        for s in range(self.nspecies):
            s_ion = f'ionization_{self.species[s]}'
        ...
        self.soln['Kn_hb'] = self.soln['mfp_hb']/self.soln['DlnP']
...
#                                     +0.01184*(self.soln['T'])**(1./3.))
        ...
        erf = np.zeros_like(self.soln['r'])
        P = self.soln['n_tot']*1.380658E-16*self.soln['T']
        P_transition = 100 #100 nano bars. Ensures that error func drops off at appropriate pressure so 
        #                    boloheat and cool are negligible inside of wind as they should be
        for i in range(len(erf)):
            erf[i] = math.erf(P[i]/P_transition)
        kappa_opt = 4e-3*erf; kappa_IR = 1e-2*erf  #need pressure BC (smooth using erf) because kappas are not valid in wind
        ...
        self.soln['heat_advect'] *= np.gradient(self.soln['e_therm'],
                                                self.soln['r'])
    ...
    def calc_massloss(self):
        ...
        self.Mdot = (4*np.pi*self.soln['r'].iloc[indx]**2
                     *self.soln['rho'].iloc[indx]*self.soln['v'].iloc[indx])
        return
    ...
    def calc_Coriolis(self):
        ...
        if self.cori.status:
            ...
            self.r_cori = optimize.fsolve(calc_vel_deflect_one_rad, self.r_crit,
                                         args=(phi_0))[0]
        return
    ...
    def calc_vert_extent(self):
        def verticle_extent(h, e_int_leftover=0.0):
            ...
            return (therm_term + q*(1./h-1./self.r_crit)
                    +1./np.sqrt(h**2 + self.semimajor_normed**2)
                    -1./np.sqrt(self.r_crit**2 + self.semimajor_normed**2))
        
        self.vert_extent = optimize.fsolve(verticle_extent, self.r_crit)[0]
        return
...
#         self.Ys_q_norm_fit = interpolate.InterpolatedUnivariateSpline(     #FIX
#             self.soln_norm['q'], self.soln_norm['Ys'], ext=2, k=degree)
#         self.Ncol_q_norm_fit = interpolate.InterpolatedUnivariateSpline(
#             self.soln_norm['q'], self.soln_norm['Ncol'], ext=2, k=degree)
    ...
    def regrid(self, q_arr=None, simple=True):
        ...
        def Ncol_q_norm_fit(Ncol_name,q):
...
#             self.Ncol_q_norm_fit(q_arr),
        ...
        )), columns=self.soln_norm.columns[:8])
