...
#         self.soln['mu'] = const.mH/mu_denominator
        ...
        self.R_hill = ((self.semimajor/self.Rp)*(self.Mp/(3.*self.Mstar))**(1./3.))
        if self.R_sp >=  0.95*self.R_hill:
            if self.print_warnings==True:
                print(f'Warning: Sonic point ({self.R_sp:.2f} Rp) may be unphysically close to R_hill ({self.R_hill:.2f} Rp).')
...
#             self.calc_Jeans()
    ...
    def add_user_vars(self,expedite=False):
        ...
        if expedite==False:
            ...
            fe = np.genfromtxt(filepath,delimiter=',',
                               usecols=(0,1,2,3))[0:10]
            rrec = np.genfromtxt(filepath,delimiter=',',
                                 skip_header=10,usecols=(0,1,2,3))[0:349]
            rnew = np.genfromtxt(filepath,delimiter=',',
                                 skip_header=359,usecols=np.arange(6))
            filepath = pkg_resources.files('wind_ae.McAstro.atoms').joinpath('Verner.csv')
        ...
        for s, species in enumerate(self.species): #48s
            ...
            heatfrac_matrix[E0_matrix<6.408707e-11] = 1 #zeroing where E_0 too low for secondary ionizations (40eV)
        ...
        if expedite == False:
            ...
            self.soln['Kn_hb_HI'] = self.soln['mfp_hb']/self.soln['DlnP'] #divided by scalelength of system
        ...
        #             self.soln[Kn_ion_name] = self.soln[L_ion_name]/self.soln['DlnP']
            ...
            filepath = pkg_resources.files('wind_ae.wrapper.wrapper_utils').joinpath('line_cooling_coeffs.dat')
        ...
        self.soln['cool_cond'] += (grad_T*(2*kappa/self.soln['r']
                                           +np.gradient(kappa, self.soln['r'])))
        try:
            cond_idx = len(np.where(abs(self.soln['cool_cond']/self.soln['heat_ion'])>0.1)[0])
            if cond_idx > 150:
                if self.print_warnings==True:
                    print("Warning: Post-facto calculations show conductive cooling is significant in this planet.\n         Wind-AE does not currently model conductive cooling.")
        except IndexError:
            pass
    #         ## collisional cooling   
    ...
    def calc_mu(self):
        ...
        for i in range(len(self.smoothing_erf)):
            ...
            self.smoothing_erf_mu[i] = 1-math.erf((v[i]-self.erf_drop[0])/(self.erf_drop[1]/4))
        ...
        for i,r_idx in enumerate(rs):
            ...
            for wl in range(len(self.E_wl)):
                ...
                for j,species in enumerate(self.species_list):
                    ...
                    Ncol = self.soln['Ncol_'+species][r_idx] #some point in wind - can experiment
