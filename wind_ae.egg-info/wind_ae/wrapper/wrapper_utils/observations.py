...
#                     #print(levelpop)
#                     #my_ray.n_abs = self.windsoln.n_neutral_fit(rs)*abund*levelpop
#                     my_ray.n_abs = self.n_neutral_fit(rs)*levelpop
#                     #print(my_ray.n_abs)
#                 elif species=='o_1':
#                     levelpop = np.zeros(len(rs))
#                     for ppp in range(len(rs)):
...
#                         bestindex = np.argmin(np.fabs(np.array(pop_s - rs[ppp])))
#                         levelpop[ppp] = levelpops[bestindex]
#                     my_ray.n_abs = self.n_ion_fit(rs)*abund*levelpop
#                 else:
#                 my_ray.n_abs = self.windsoln.n_neutral_fit(rs)
...
#         else:
#         my_ray.n_abs = np.ones(len(s))*np.array(self.windsoln.soln['n_'+sp])[-1]
        my_ray.ds *= (s[1]-s[0])*self.windsoln.Rp
