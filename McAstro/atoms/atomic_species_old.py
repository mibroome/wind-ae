...
#             if str(ion_num[-1]) not in ['M', 'D', 'C', 'L', 'X', 'V', 'I']:
#                 roman = arabic_to_roman(int(ion_num))
#                 species_list = element+' '+str(roman) 
#             else:
#                 species_list = name
#         return species_list
...
class atomic_species:
    ...
    def cross_section(self, E, units='cm^2', valid_range=True):
        ...
        if (self.Z in [6,7,8,9,12,14,16]): #x-rays photoionize certain species' K shell electrons instead
            y = E/self.verner_data['E_0']
            l=0 #K shell (l=0)
            Q = 5.5+l-0.5*self.verner_data['P']
            Fy_xray = (((y-1)**2+self.verner_data['y_w']**2)*y**(-Q) 
                 *(1+np.sqrt(y/self.verner_data['y_a']))**(-self.verner_data['P'])) #Verner & Yakovlev (1995)
            high_E = kshell_ionpots[self.Z] #setting Kshell ionization energy
#             print(high_E)
#             print('fy', Fy[E>=high_E])
#             print('fy xray',Fy_xray[E>=high_E])
            self.verner_data['E_max'] = np.max(E) #raises the ceiling included in the Verner data 
            Fy[E>=high_E] = Fy_xray[E>=high_E] #above the k shell ionization threshold, use K shell cross-sec
