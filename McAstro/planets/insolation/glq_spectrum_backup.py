...
class glq_spectrum:
    ...
    def __init__(self, filename='',lisird=True, mission='fism2', date='2002-01-01', wl_norm=1e-7):
        ...
        if lisird == True:
            ...
            spectrum = pd.read_csv('../../stars/spectrum/additional_spectra/'+filename,comment='#')
            self.spectrum = spectrum
            self.data = spectrum.data
            if self.data.iloc[-1]['wl'] > 1e-4:
                print("WARNING: Wavelengths may not be in units of cm. Please double check all columns are in cgs units.")
...
#         print("Min in nm from glq_spectrum.py",np.min(self.data_norm['wl']))
...
#         print("truncate_spectrum has been activated and, therefore, bin_spectrum(new_breaks).")
...
#             if np.shape(self.data_norm.loc[mk, var])[0] > savgol_window: #new species bin edges are making too small windows
            #this is a kludge to get it to stop. Would prefer not to add too close bin edge in the first place
#             print('x',np.shape(self.data_norm.loc[mk, var]))
...
#         print(self.wndw_span)
        ...
        Phi_smth_tot = 0.
#         print("write_csv n_bins:",self.n_bins)
        for b in range(self.n_bins):
