...
while keep_going == True:
    try:
        sim.load_planet(f'starts/{mass_grid[node][r_idx]:.2f}Me_{radius_array[r_idx]:.2f}Re.csv',
...
for i in range(1,len(radius_array)):
    ...
    if result == 0:
        ...
        sim.save_planet(savepath+f'{actual_Mp:.2f}Me_{actual_Rp:.2f}Re.csv',overwrite=True)
