...
def custom_rc_params(line_color,nspecies):
...
def quick_plot(soln, Mdot_legend=True, line_color='k', line_style='-', line_label='',label_dim=[0,1.3,2],
    ...
    ax[0,1].plot(soln.soln_norm['r'], soln.soln['T']/1000,c=line_color,
                     ls=line_style,label=r'$\dot{M}$ = %.1e g/s' %(mdot))
...
def six_panel_plot(soln,Mdot_legend=True,line_color='k',line_style='-',line_label='',
                   label_dim=[0,1.3,2],ion_label=True,first_plotted=True,ax=0): 
    '''
    Description: 
        Plots density (g/cm3), temperature (K), velocity (10 km/s), 
        ionization fraction, column density (g/cm2), and number density (1/cm2),
        as a function of r (Rp).
    ...
    if len(line_label) == 0:
        ...
        line_label = f'{Mstr:.2f}Mearth_{Rstr:.2f}Rearth_{Fstr:s}F_{spectrum:s}'
    radius_norm = 1.0
    alpha=0.5
    radius = soln.soln_norm['r']
    minrad = float(radius[soln.soln['T'] == np.min(soln.soln['T'])])
    ...
    mdot = (soln.Mdot)#/const.Msun)*3.154e+7 #in Msun/year
    print(f'***** {line_label:s}: Mdot = {mdot:.2e} g/s *****')
    ax[2,0].plot(soln.soln_norm['r'], soln.soln['T']/1e3,c=line_color,
                 ls=line_style,label=r'$\dot{M}$ = %.1e g/s' %(mdot))
