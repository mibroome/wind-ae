...
class physics:
    ...
    def __init__(self, HX, species_list, molec_adjust, atomic_masses=np.array([0])):
        ...
        Ne = np.zeros_like(HX)
        for j in range(len(species_list)):
            Z[j] = atomictable['Z'][atomictable['Name']==(species_list[j]).replace(' ','')]
            Ne[j] = atomictable['N_electrons'][atomictable['Name']==(species_list[j]).replace(' ','')]
        self.Z = Z
        self.Ne = Ne 
    ...
    def calc_mu(self):
#         print(self.windsoln.species_list[0])
#         if (self.windsoln.species_list[0] != "H I") and (self.windsoln.species_list[0] != "HI"):
